(() => {
	...
	const setupAdmin = () => {
		...
		const renderLineChart = (sales, profit) => { const values = [...sales, ...profit]; const scale = chartScales(values); const width = Math.max(1, sales.length - 1); const points = (series) => series.map((value, index) => `${40 + (index / width) * 570},${scale.y(value)}`).join(' '); const labels = sales.map((_, index) => `<text x="${40 + (index / width) * 570}" y="258" text-anchor="middle">${index + 1}</text>`).join(''); mountChart('salesProfitChart', svgShell(`<g class="chart-grid"><path d="M40 42H610M40 137H610M40 232H610"></path></g><polyline class="chart-line sales-line" points="${points(sales)}"></polyline><polyline class="chart-line profit-line" points="${points(profit)}"></polyline>${sales.map((value, index) => `<circle class="chart-point sales-point" cx="${40 + (index / width) * 570}" cy="${scale.y(value)}" r="4" data-tooltip="Sales: ${formatMoney(value)}"></circle><circle class="chart-point profit-point" cx="${40 + (index / width) * 570}" cy="${scale.y(profit[index])}" r="4" data-tooltip="Profit: ${formatMoney(profit[index])}"></circle>`).join('')}<g class="chart-labels">${labels}</g>`, 'Sales and estimated profit trend')); };
		...
		const renderDonut = (groups) => { const total = groups.reduce((sum, group) => sum + group.value, 0) || 1; let offset = 0; const rings = groups.map((group, index) => { const dash = (group.value / total) * 100; const result = `<circle class="donut-segment" cx="140" cy="140" r="82" pathLength="100" stroke="${chartColors[index % chartColors.length]}" stroke-dasharray="${dash} ${100 - dash}" stroke-dashoffset="${-offset}" data-tooltip="${group.name}: ${formatMoney(group.value)}"></circle>`; offset += dash; return result; }).join(''); const legend = groups.map((group, index) => `<text x="290" y="${85 + index * 28}"><tspan fill="${chartColors[index % chartColors.length]}">●</tspan> ${escapeHtml(group.name)} ${Math.round((group.value / total) * 100)}%</text>`).join(''); mountChart('categoryDonutChart', svgShell(`<g class="donut-chart"><circle class="donut-base" cx="140" cy="140" r="82"></circle>${rings}<text class="donut-total" x="140" y="137" text-anchor="middle">${formatMoney(total)}</text><text class="donut-caption" x="140" y="158" text-anchor="middle">sales</text>${legend}</g>`, 'Sales by product category')); };
		const renderStacked = (ordersInWindow) => { const status = [{ name: 'Completed', value: ordersInWindow.length }, { name: 'Pending review', value: ordersInWindow.length ? Math.ceil(ordersInWindow.length * 0.15) : 0 }]; const total = Math.max(status.reduce((sum, item) => sum + item.value, 0), 1); let x = 40; const bars = status.map((item, index) => { const width = (item.value / total) * 570; const markup = `<rect class="stack-bar" x="${x}" y="100" width="${width}" height="80" fill="${chartColors[index + 1]}" data-tooltip="${item.name}: ${item.value}"></rect>`; x += width; return markup; }).join(''); const legend = status.map((item, index) => `<text x="${45 + index * 170}" y="220"><tspan fill="${chartColors[index + 1]}">●</tspan> ${item.name} ${item.value}</text>`).join(''); mountChart('statusStackedChart', svgShell(`${bars}${legend}`, 'Order status stacked bar')); };
		const renderHistogram = (ordersInWindow) => { const values = ordersInWindow.map((order) => Number(order.total) || 0); const maxValue = Math.max(...values, 100); const bucketCount = 6; const counts = Array(bucketCount).fill(0); values.forEach((value) => counts[Math.min(bucketCount - 1, Math.floor((value / maxValue) * bucketCount))]++); const scale = chartScales(counts); const bars = counts.map((count, index) => { const width = 570 / bucketCount - 8; const height = (count / scale.max) * 190; return `<rect class="bar histogram-bar" x="${45 + index * (570 / bucketCount)}" y="${232 - height}" width="${width}" height="${height}" data-tooltip="${count} order${count === 1 ? '' : 's'} in ${formatMoney((index / bucketCount) * maxValue)}-${formatMoney(((index + 1) / bucketCount) * maxValue)}"></rect><text x="${45 + index * (570 / bucketCount) + width / 2}" y="258" text-anchor="middle">${Math.round((index / bucketCount) * maxValue / 100) * 100}</text>`; }).join(''); mountChart('orderHistogramChart', svgShell(`<g class="chart-grid"><path d="M40 42H610M40 137H610M40 232H610"></path></g>${bars}`, 'Order value histogram')); };
		...
		const renderAnalytics = () => { const windowOrders = filteredOrders(); const margin = Number(document.querySelector('#profitMargin').value) / 100; const revenue = windowOrders.reduce((sum, order) => sum + (Number(order.total) || 0), 0); const profit = revenue * margin; const average = windowOrders.length ? revenue / windowOrders.length : 0; const units = windowOrders.reduce((sum, order) => sum + (order.items || []).reduce((itemSum, item) => itemSum + Number(item.quantity || 0), 0), 0); analyticsKpis.innerHTML = `<div class="analytics-kpi"><span>Revenue</span><strong>${formatMoney(revenue)}</strong><small>Selected period</small></div><div class="analytics-kpi"><span>Estimated profit</span><strong>${formatMoney(profit)}</strong><small>${Math.round(margin * 100)}% model</small></div><div class="analytics-kpi"><span>Orders</span><strong>${windowOrders.length}</strong><small>Checkout enquiries</small></div><div class="analytics-kpi"><span>Average order</span><strong>${formatMoney(average)}</strong><small>${units} tracked units</small></div>`; const points = Number(document.querySelector('#analyticsGranularity').value === 'monthly' ? 12 : 8); const salesSeries = Array.from({ length: points }, (_, index) => windowOrders.filter((order) => { const date = new Date(order.createdAt); return (document.querySelector('#analyticsGranularity').value === 'monthly' ? date.getMonth() : Math.floor(date.getDate() / 7)) % points === index; }).reduce((sum, order) => sum + Number(order.total || 0), 0)); renderLineChart(salesSeries, salesSeries.map((value) => value * margin)); const performance = {}; windowOrders.forEach((order) => (order.items || []).forEach((item) => { performance[item.name] ||= 0; performance[item.name] += Number(item.price || 0) * Number(item.quantity || 0); })); const productData = Object.entries(performance).map(([name, sales]) => ({ name, sales, profit: sales * margin })).sort((a, b) => b.sales - a.sales).slice(0, 8); renderClusteredBars(productData.length ? productData : [{ name: 'No orders yet', sales: 0, profit: 0 }]); const categories = {}; windowOrders.forEach((order) => (order.items || []).forEach((item) => { const category = categoryFor(item.name); categories[category] = (categories[category] || 0) + Number(item.price || 0) * Number(item.quantity || 0); })); renderDonut(Object.entries(categories).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value).slice(0, 5).length ? Object.entries(categories).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value).slice(0, 5) : [{ name: 'No sales yet', value: 0 }]); renderStacked(windowOrders); renderHistogram(windowOrders); renderScatter(); };
